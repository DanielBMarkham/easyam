module Persist
        ...
        let ig = match x.Location.Bucket with
                    | Buckets.Behavior->
                        ...
                        let affectedByEx = 
                            if ((getALL_MUS modelItems).IsNone)
                                then
                                    let ret=isAffectedBy|>Array.map(fun z->getModelItemDescForId modelItems z.TargetId)
                                    ret
                                else
                                    let allIsAffectedBy = getAffectedBy (getALL_MUS modelItems).Value
                                    let temp=if x.Description.Trim()<>"ALL" then (allIsAffectedBy |> Array.append isAffectedBy) else allIsAffectedBy
                                    let ret=temp|>Array.map(fun z->getModelItemDescForId modelItems z.TargetId)
                                    ret
                        if affectedByEx.Length=0 then () else
                            ...
                            sb.wl("")
                        let usesEntities = (getUses x)
                        let usesEntitiesEx = usesEntities|>Array.map(fun z->getModelItemDescForId modelItems z.TargetId)
                    ...
                    | Buckets.Structure->
                        let isUsedBy= (getUsedBy x)
                        let isUsedByEx=isUsedBy|>Array.map(fun z->getModelItemDescForId modelItems z.TargetId)
                        let hasA = (getHasA x)
                        let hasAEx = hasA |>Array.map(fun z->getModelItemDescForId modelItems z.TargetId)
                    ...
                    | Buckets.Supplemental->
                        let affects= (getAffects x)
                        let affectsEx=affects|>Array.map(fun z->getModelItemDescForId modelItems z.TargetId)
    ...
    let saveFeatureFile (fileName:string) (musItem:ModelItem) (compilerStatus:CompilerReturn) =
        ...
        sb.wl ("")
        writeIndentedTextDetailsWithAHeading sb 1 2 "" ((getNotes musItem) |> Array.map(fun z->z.AnnotationText))   //(musItem.Annotations|>Array.filter(fun x->(fst x)=ANNOTATION_TOKEN_TYPE.Note)|>Array.map(fun x->(snd x)))
        sb.wl ("")
        writeIndentedTextDetailsWithAHeading sb 1 2 "Open question" ((getQuestions musItem) |> Array.map(fun z->z.AnnotationText)) //(musItem.Annotations|>Array.filter(fun x->(fst x)=ANNOTATION_TOKEN_TYPE.Question)|>Array.map(fun x->(snd x)))
        sb.wl ("")
        writeIndentedTextDetailsWithAHeading sb 1 2 "Diagram" ((getDiagrams musItem) |> Array.map(fun z->z.AnnotationText)) //(musItem.Annotations|>Array.filter(fun x->(fst x)=ANNOTATION_TOKEN_TYPE.Diagram)|>Array.map(fun x->(snd x)))
        sb.wl ("")
        ...
        scenarios|>Array.iter(fun x->
            ...
            let scenarioNotes=(getAttributeNotes x) |> Array.map(fun z->z.AnnotationText) //x.Annotations|>Array.filter(fun y->(fst y)=ANNOTATION_TOKEN_TYPE.Note)|>Array.map(fun y->(snd y))
            writeIndentedTextDetailsWithAHeading sb 1 2 "" scenarioNotes
            sb.wl ("")
            let scenarioQuestions=(getAttributeNotes x) |> Array.map(fun z->z.AnnotationText)  //x.Annotations|>Array.filter(fun y->(fst y)=ANNOTATION_TOKEN_TYPE.Question)|>Array.map(fun y->(snd y))
            writeIndentedTextDetailsWithAHeading sb 1 2 "Open question" scenarioQuestions
            sb.wl ("")
            let scenarioDiagrams=(getAttributeNotes x) |> Array.map(fun z->z.AnnotationText)  //x.Annotations|>Array.filter(fun y->(fst y)=ANNOTATION_TOKEN_TYPE.Diagram)|>Array.map(fun y->(snd y))
    ...
    let writeJoinDetail (sb:System.Text.StringBuilder) (referenceModel:ModelItem []) (outputFormat:ModelOutputType) (modelItem:ModelItem) (detailLevel:ModelDetailLevelFlags) =
